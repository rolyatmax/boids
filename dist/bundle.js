...
var _slicedToArray = function () { function sliceIterator(arr, i) { var _arr = []; var _n = true; var _d = false; var _e = undefined; try { for (var _i = arr[Symbol.iterator](), _s; !(_n = (_s = _i.next()).done); _n = true) { _arr.push(_s.value); if (i && _arr.length === i) break; } } catch (err) { _d = true; _e = err; } finally { try { if (!_n && _i["return"]) _i["return"](); } finally { if (_d) throw _e; } } return _arr; } return function (arr, i) { if (Array.isArray(arr)) { return arr; } else if (Symbol.iterator in Object(arr)) { return sliceIterator(arr, i); } else { throw new TypeError("Invalid attempt to destructure non-iterable instance"); } }; }(); /* global requestAnimationFrame cancelAnimationFrame */

var _alea = require('alea');

var _alea2 = _interopRequireDefault(_alea);

var _resizableCanvas = require('./lib/resizable-canvas');

var _resizableCanvas2 = _interopRequireDefault(_resizableCanvas);

var _colorPalettes = require('./lib/color-palettes.json');
...
var rand = new _alea2.default(seed);

var container = document.createElement('div');
document.body.appendChild(container);

var canvas = (0, _resizableCanvas2.default)(container, main, { margin: 0 });
...
function update(t) {
  ...
  boids = boids.map(function (boid) {
    return align(boid, boidsByColor[boid.color]);
  });
  boids = boids.map(function (boid) {
    return avoid(boid, boidsByColor[boid.color]);
  });
  boids = boids.map(function (boid) {
    return cohere(boid, boidsByColor[boid.color]);
  });

  boids = boids.map(function (boid) {
    var _add = add(boid.position, multiply(boid.velocity, speed)),
...
function align(boid, boids) {
  ...
  boids.forEach(function (b) {
    var distance = dist(b.position, boid.position);
  ...
  sum = normalize(sum);
  var steer = subtract(sum, boid.velocity);
  steer = normalize(steer);
  steer = multiply(steer, settings.agility);

  var velocity = add(boid.velocity, steer);
...
function cohere(boid, boids) {
  ...
  sum = divide(sum, neighbors);
  var desired = subtract(sum, boid.position);
  desired = normalize(desired);
  var steer = subtract(desired, boid.velocity);
  steer = normalize(steer);
  steer = multiply(steer, settings.agility);
...
function avoid(boid, boids) {
  ...
  boids.forEach(function (b) {
    ...
    if (distance > 0 && distance < settings.separation) {
      var diff = subtract(boid.position, b.position);
      diff = normalize(diff);
...
function normalize(vec) {
  var len = dist([0, 0], vec);
  if (!len) return vec;
  return divide(vec, len);
}

function dist(vecA, vecB) {
  var xDiff = vecA[0] - vecB[0];
  var yDiff = vecA[1] - vecB[1];
...
function drawBoid(boid) {
  var _boid$position = _slicedToArray(boid.position, 2),
      x = _boid$position[0],
      y = _boid$position[1];

  var _normalize = normalize(boid.velocity),
...
function changeColors() {
  colors = _colorPalettes2.default[rand() * _colorPalettes2.default.length | 0].slice(0, 3);
  boids.forEach(function (boid) {
...
var gui = new _datGui.GUI({ closed: true });
...
gui.add({ changeColors: changeColors }, 'changeColors').onFinishChange(main);
gui.add({ reset: reset }, 'reset');

},{"./lib/color-palettes.json":3,"./lib/resizable-canvas":4,"alea":5,"dat-gui":6}],2:[function(require,module,exports){
"use strict";

Object.defineProperty(exports, "__esModule", {
  value: true
});
exports.default = applyAspectRatio;
function applyAspectRatio(containerDimensions, aspectRatio) {
...
exports.default = createResizableCanvas;

var _underscore = require('underscore');

var _applyAspectRatio2 = require('./apply-aspect-ratio');

var _applyAspectRatio3 = _interopRequireDefault(_applyAspectRatio2);

function _interopRequireDefault(obj) { return obj && obj.__esModule ? obj : { default: obj }; }

function createResizableCanvas(container, onResize, opts) {
  var _opts$margin = opts.margin,
      margin = _opts$margin === undefined ? 0 : _opts$margin,
      aspectRatio = opts.aspectRatio;

  var canvas = document.createElement('canvas');
  container.appendChild(canvas);
  var getMargin = typeof margin === 'function' ? margin : function () {
    return margin;
  };
  ...
  function size() {
    ...
    var canvasAspectRatio = aspectRatio || parent.width / parent.height;

    var _applyAspectRatio = (0, _applyAspectRatio3.default)(parent, canvasAspectRatio),
        height = _applyAspectRatio.height,
        width = _applyAspectRatio.width;
...
},{"./apply-aspect-ratio":2,"underscore":9}],5:[function(require,module,exports){
